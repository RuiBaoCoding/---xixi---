...
int B_PLUS_TREE_LEAF_PAGE_TYPE::KeyIndex(const KeyType &key, const KeyComparator &comparator) const {
  ...
  int left = 0;
  int right = s - 1;
  while (left <= right) {//[left,right]÷–ªπ”–‘™Àÿ
    int mid = (right + left) / 2;
    if (comparator(key,KeyAt(mid))<=0) {  // Õ˘◊Û±ﬂÀ—
      right = mid - 1;
    } else {  // Õ˘”“±ﬂÀ—
      left = mid + 1;
    }
  ...
  return GetSize();*///Ëã•‰∏çÂ≠òÂú®ÊØîkeyÂ§ßÁöÑÂÄºÔºåËøîÂõûsize_
}
/*
...
int B_PLUS_TREE_LEAF_PAGE_TYPE::RemoveAndDeleteRecord(const KeyType &key, const KeyComparator &comparator) {
  ...
  IncreaseSize(-1);//Remove‰πãÂêésize--
  return GetSize();
}
